...
git config --local --get include.path 2>/dev/null | grep -q '1_configs/dist/gitconfig' \
    || fail ".git/config missing [include] for 1_configs/dist/gitconfig"
pass ".git/config [include] wired"

# 2. hooksPath resolves to 1_configs/dist/hooks
...
[ -x "1_configs/dist/hooks/pre-commit" ] \
...
grep -q "P4.*env" "1_configs/dist/hooks/pre-commit" \
